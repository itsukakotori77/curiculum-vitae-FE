...
import GeneratorForm3, {
  GeneratorForm3Ref,
} from './GeneratorForm3'
import moment from 'moment'
import GeneratorForm4, {
  GeneratorForm4Ref,
} from './GeneratorForm4'
import GeneratorForm5, {
  GeneratorForm5Ref,
} from './GeneratorForm5'
...
export default function CurrVitaeGenerator() {
  ...
  const { contacts, update: updateContacts } =
    useCVStep5Store()
              ...
              <div className="flex flex-col">
                ...
                <p className="text-sm">{`${
                  data.graduatedStatus === 'true'
                    ? `Graduated From ${moment(
                        data?.graduated,
                      ).format('MMMM, Do YYYY')}`
                    : `Undergraduated`
                }`}</p>
                <p className="text-sm">{`${
                  data.gpa ? `GPA: ${data?.gpa}` : `Non GPA`
                }`}</p>
                <p className="text-sm text-gray-600 mt-1">
                  {data.majorDesc}
                </p>
