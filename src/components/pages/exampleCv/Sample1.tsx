...
import { faEnvelope, faLink, faMapPin, faPhone } from '@fortawesome/free-solid-svg-icons'
...
const cvVariants = cva(
  ...
  "flex w-full bg-white shadow-lg min-h-fit print:shadow-none",
  {
    variants: {
      ...
      printable: {
        print: ['min-h-screen', 'flex', 'justify-center', 'items-start', 'print:bg-white', 'print:py-0'],
        ...
        >
            ...
            <div className="flex flex-col p-4 gap-y-2">
              ...
              {data?.contacts?.phone && (
                <div className="flex flex-row gap-2 items-center w-full justify-start">
                  ...
                  <CVText variant="tiny" size={textSize} className="font-semibold text-gray-500">
                    {data.contacts.phone}
                  </CVText>
                </div>
              )}
          ...
          >
            <div className="flex flex-col gap-1 mb-6">
              <CVText variant="title" size={textSize} className="text-[#5977AC]">
            ...
            <div className="flex my-4 mx-2">
              <ol className="relative border-s border-gray-200">
                ...
                <li className="mb-8 ms-6">
                  ...
                  <CVText variant="subtitle" size={textSize} className="flex items-center mb-3 text-gray-900">
                    Experience
                  </CVText>
                  ...
                  <ul className="space-y-4 list-disc pl-4">
                    {data?.experience?.toReversed()?.map((item: any, key: number) => (
                      <li key={key}>
                        <CVText variant="small" size={textSize} className="block mb-2 font-bold leading-none text-gray-600">
                          {`${item?.jobTitle} - ${item?.company}`}
                        </CVText>
                        <CVText variant="small" size={textSize} className="flex items-center mb-2 font-medium leading-none text-gray-500">
                          {moment(item?.startDate).format('MMMM, Do YYYY')}
                          <ChevronRight className="w-4 h-4 mx-1" />
                          {item?.isCurrent ? 'Current' : moment(item?.endDate).format('MMMM, Do YYYY')}
                        </CVText>
                        <CVText variant="small" size={textSize} className="mb-2 font-normal text-gray-500 leading-relaxed">
                          {item?.descJob}
                        </CVText>
                      </li>
                    ))}
