...
const ModalConfirm = () => {
   ...
   const [state, setState] = useState<TypeModalConfirm & { isOpen: boolean }>({
      isOpen: false,
      title: '',
      cancelText: 'No',
      submitText: 'Yes',
   ...
   useEffect(() => {
      ...
      emitter.on('loading-modal-confirm', (val: boolean) => setLoading(val))
   ...
   return (
      ...
      >
         <div className="flex justify-center items-center flex-col mt-2 mb-4 gap-2">
            ...
            <span className="text-lg font-semibold">{state.description}</span>
         </div>
         ...
         <div className="flex justify-between gap-5">
            ...
            ><span className="font-bold">{state.submitText || 'Yes'}</span>
            </Button>
            <Button
               intent="default"
               isLoading={loading}
               onClick={closeModal}
            ><span className="font-bold">{state.cancelText || 'No'}</span>
            </Button>
         </div>
      </Modal>
   )
