      ...
      return (
         ...
         >
            ...
            >
               ...
               <div
                  className={`flex space-x-1 p-3 rounded-lg border-2 transition-colors duration-200 ${getBorderColor()} ${isDisabled ? 'opacity-50 cursor-not-allowed bg-gray-50' : 'bg-white hover:border-blue-300'
                     }`}
               ...
               >
                  {[1, 2, 3, 4, 5].map((starValue: number) => (
                     <motion.button
                        ...
                        disabled={isDisabled}
                        className={`p-1 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 rounded ${isDisabled ? 'cursor-not-allowed' : 'cursor-pointer'
                           }`}
                        whileHover={isDisabled ? {} : { scale: 1.15 }}
                        whileTap={isDisabled ? {} : { scale: 0.95 }}
                        initial={{ opacity: 0, y: 10 }}
                        animate={{ opacity: 1, y: 0 }}
                        transition={{ delay: starValue * 0.05, duration: 0.3 }}
                        aria-label={`Rate ${starValue} star${starValue !== 1 ? 's' : ''}`}
                     >
                        <motion.div
                           animate={{
                              rotate: !isDisabled && (hoveredRating === starValue || value === starValue) ? 360 : 0,
                           }}
                           transition={{
                              type: "spring",
                              stiffness: 300,
                     ...
                     >
                        ...
                        >
                           <motion.div
                              ...
                              initial={{ width: 0 }}
                              animate={{ width: `${((hoveredRating || value) / 5) * 100}%` }}
