...
const switchVariants = cva(
   ...
   rounded-full relative cursor-pointer transition duration-200`,
   {
      variants: {
         intent: {
            primary: 'data-[checked=true]:bg-blue-500 data-[checked=false]:bg-slate-700 data-[checked=false]:border-slate-500',
            secondary: 'data-[checked=true]:bg-green-500 data-[checked=false]:bg-slate-700 data-[checked=false]:border-slate-500',
            warning: 'data-[checked=true]:bg-yellow-500 data-[checked=false]:bg-slate-700 data-[checked=false]:border-slate-500',
            danger: 'data-[checked=true]:bg-red-500 data-[checked=false]:bg-slate-700 data-[checked=false]:border-slate-500',
...
interface IProps extends Omit<HTMLMotionProps<'input'>, 'size'>,
...
const Switch = forwardRef<HTMLInputElement, IProps>(
   ({
      ...
      const handleChecked = useCallback((val: boolean) => {
         onChangeSwitch(val)
      }, [onChangeSwitch])
            ...
            >
               <motion.div
                  initial={{
                     ...
                     x: isChecked ? config.translateX.unchecked : config.translateX.checked,
                  }}
                  animate={{
                     height: config.animateHeight,
                     width: config.animateWidth,
                     x: isChecked ? config.translateX.checked : config.translateX.unchecked,
                  }}
                  transition={{
                     duration: 0.3,
                     delay: 0.1,
