...
const ColorPickerInput = forwardRef<HTMLDivElement, ColorPickerProps>(
   ({ isDisabled, isInvalid, isValid, value, onChange, onBlur, name, ...props }, ref) => {
      // Default color value
      const defaultColor: RgbaColor = { r: 255, g: 255, b: 255, a: 1 }
      
      const [color, setColor] = useState<RgbaColor>(value || defaultColor)
         ...
         >
            ...
            <div 
               className={`w-full h-10 rounded border-2 mb-2 ${
                  isInvalid ? 'border-red-500' : isValid ? 'border-green-500' : 'border-gray-300'
               }`}
